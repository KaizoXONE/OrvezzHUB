...
do
	...
	-- parameter validation for table-based config
	function utility:ValidateConfig(config, schema)
		config = config or {}
		
		-- Check required fields
		for key, rules in pairs(schema) do
			if rules.Required and config[key] == nil then
				error(string.format("[UILibrary] Missing required parameter: '%s'", key))
			end
			
			-- Type validation
			if config[key] ~= nil and rules.Type then
				local actualType = typeof(config[key])
				if actualType ~= rules.Type then
					error(string.format("[UILibrary] Invalid type for '%s': expected %s, got %s", 
						key, rules.Type, actualType))
				end
			end
			
			-- Set default value
			if config[key] == nil and rules.Default ~= nil then
...
do
	...
	function library.new(title, icon, config)
		...
		-- Create floating icon for minimize/maximize
		local floatingIcon = utility:Create("ImageButton", {
			Name = "FloatingIcon",
			Parent = container,
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundTransparency = 1,
		...
		-- Track dragging state to prevent toggle on drag
		local isDragging = false
		local dragStartTime = 0
		
		floatingIcon.MouseButton1Down:Connect(function()
			dragStartTime = tick()
			isDragging = false
		end)
		...
		floatingIcon.InputChanged:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseMovement then
				if tick() - dragStartTime > 0.05 then
					isDragging = true
				end
			end
		end)
	...
	function library:Notify(title, text)
		...
		local textSize = game:GetService("TextService"):GetTextSize(text or "", 12, Enum.Font.Gotham, Vector2.new(math.huge, 18))
		
		-- initial position
		local padding = 10
		notification.Position = UDim2.new(0, padding, 1, -70)
		notification.Size = UDim2.new(0, 0, 0, 60)
		
		-- animate in
		utility:Tween(notification, {Size = UDim2.new(0, math.max(textSize.X + 30, 150), 0, 60)}, 0.2)
		...
		local close = function()
			...
			utility:Tween(notification, {
				...
				Position = notification.Position + UDim2.new(0, notification.Size.X.Offset, 0, 0)
			}, 0.2)
	...
	function section:Button(config)
		...
		local button = utility:Create("ImageButton", {
			...
			utility:Create("TextLabel", {
				Name = "Title",
				BackgroundTransparency = 1,
				Position = UDim2.new(0, (iconName and 34 or 10), 0, 0),
				Size = UDim2.new(1, -(iconName and 44 or 10), 1, 0),
				ZIndex = 3,
				Font = Enum.Font.Gotham,
				Text = title,
				TextColor3 = themes.TextColor,
		...
		button.MouseButton1Click:Connect(function()
			...
			debounce = true
			local originalSize = text.TextSize
			text.TextSize = 0
			utility:Tween(text, {TextSize = originalSize + 2}, 0.2)
			
			wait(0.2)
			utility:Tween(text, {TextSize = originalSize}, 0.2)
	...
	function section:addButton(title, callback)
		warn("[UILibrary] addButton is deprecated, use Button({Title, Callback}) instead")
		return self:Button({
			Title = title,
			Callback = callback
		})
	...
	function section:Toggle(config)
		...
		local toggle = utility:Create("ImageButton", {
			...
			utility:Create("TextLabel", {
				...
				AnchorPoint = Vector2.new(0, 0.5),
				BackgroundTransparency = 1,
				Position = UDim2.new(0, (iconName and 34 or 10), 0.5, 1),
	...
	function section:addToggle(title, default, callback)
		warn("[UILibrary] addToggle is deprecated, use Toggle({Title, Value, Callback}) instead")
		return self:Toggle({
			Title = title,
			Value = default,
			Callback = callback
		...
		local textbox = utility:Create("ImageButton", {
			...
			utility:Create("ImageLabel", {
				...
				utility:Create("TextBox", {
					...
					TextColor3 = themes.TextColor,
					TextSize = 13,  -- Medium size (increased from 11)
					ClearTextOnFocus = false  -- Disable auto-clear on focus
				})
			})
		})
	...
	function section:addTextbox(title, default, callback)
		warn("[UILibrary] addTextbox is deprecated, use Input({Title, Value, Callback}) instead")
		return self:Input({
			Title = title,
			Value = default,
			Callback = callback
	...
	function section:Dropdown(config)
		...
		local title = config.Title
		local list = config.Values
		local multi = config.Multi
		local callback = config.Callback
	...
	function section:addDropdown(title, list, callback)
		warn("[UILibrary] addDropdown is deprecated, use Dropdown({Title, Values, Callback}) instead")
